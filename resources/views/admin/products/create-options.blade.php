...
@section('title')
    {{trans('general.create')}} {{trans('general.options')}}
@endsection
...
@section('content')
    ...
    <section id="basic-horizontal-layouts">
        <form class="form form-horizontal" action="{{route('dashboard.products-options.store',$entity)}}" method="POST"
                    ...
                    <div class="form-group col-md-6 p-1">
                        ...
                        <select id="MainID" name="MainID" autocomplete="off" class="custom-select">
                            @foreach($options ??[] as $option)
                                <option onclick="getSubOptions({{$option->id}})"
                    ...
                    <div class="form-row p-1">
                        <div class="form-group col-md-6 p-1">
                            ...
                            <select name="OptID" id="OptID" autocomplete="off" class="custom-select">
                                @if($options??[] && count($options)>0)
                                    @foreach($options[0]->subOption ??[] as $option)
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input type="number" step="any" min="0" max="1000" name="AdditionalValue"
                                   value="{{old('AdditionalValue')}}" id="AdditionalValue" class="form-control">
                        </div>
                    </div>
                    <div class="form-group col-md-3 p-1">
                        <button type="submit" class="btn btn-danger">{{trans('general.save')}}</button>
                        <a href="{{route('dashboard.products.options',$entity->id)}}" class="btn btn-default">{{trans('general.back')}}</a>
                    </div>
                </div>
            </div>
        </form>
