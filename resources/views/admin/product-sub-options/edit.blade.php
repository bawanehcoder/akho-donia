...
@section('title')
    {{trans('general.update')}} {{trans('general.sub_options')}}
@endsection
...
@section('content')
    ...
    <section id="basic-horizontal-layouts">
        <form class="form form-horizontal" action="{{route('dashboard.product-sub-options.update',$entity)}}"
              method="POST"
            ...
            <div class=" card table-list-card>
                <div class=" card-body table-list-card-body">
                    <div class="form-group col-md-6 ">
                        ...
                        <select name="OptID" autocomplete="off" id="OptID" class="select2 main_categories w-100">
                            @foreach($basic_options ??[] as $option)
                                <option
                                    {{$entity->OptID==$option->id?'selected':''}} value="{{$option->id}}">{{$option->Name}}
                    ...
                    <div class="form-group col-md-6 ">
                        ...
                        <select name="Available" autocomplete="off" class="custom-select w-100">
                            <option
                                {{$entity->Available==0? 'selected' :''}}  value="0">{{trans('general.available')}}</option>
                    ...
                    <div class="form-group col-md-3 ">
                        ...
                        <a href="{{route('dashboard.product-sub-options.index' )}}"
